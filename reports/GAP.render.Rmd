...
Then, we introduce the function "get_performance" which augments a given data set with the performance of each solver with respect to the best solver.
...
The "plot_performance" function then plots the ecdf of the obtained performance.
...
plot_performance = function(dataset,
  ...
  if (main == "") {
    main = paste0("ECDF of ", column_criteria ," performance for all solvers")
  }
...
plot_performance(
  ...
  column_solver = "solver_with_params"
)
```
...
```{r}
plot_performance(results[results$solver == "idol_bap",], 
                 main = "idol",
                 log = TRUE,
                 column_solver = "solver_with_params")
```
...
plot_performance(
  ...
  main = "Stabilization by smoothing dual values: different values for alpha", 
  log = TRUE,
  column_solver = "solver_with_params"
)
```

...
For example, the following table gathers results from the Branch-and-Bound algorithm of coluna.

```{r}
Table = make_table(results[results$solver == "coluna",])
```
...
```{r}
performance_profile(results[results$solver != "external",], xlim = c(1, 10), main = "idol versus coluna (up to 10)")
performance_profile(results[results$solver != "external",], xlim = c(1, 200), main = "idol versus coluna (up to 200)")
```

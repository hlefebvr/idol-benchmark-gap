...
to test their implementation of the Branch-and-Price algorithm for solving instances of the [Generalized Assignment Problem](https://en.wikipedia.org/wiki/Generalized_assignment_problem).
...
$$
  ...
  \end{array}
$$
Here, variable $x_{ij}$ encode the assignment decision and equals $1$ if and only if task $j$ is assigned to agent $i$.

## Reading instances

We first start to read results obtained using idol, which can be found inside the `results_GAP_idol.csv` file.
...
results = rbind(results, coluna)
```

We can now print out the obtained computational results for each instance for each solver.

```{r echo = FALSE}
paged_table(results)
```

The list of all tested solvers can be obtained as follows.

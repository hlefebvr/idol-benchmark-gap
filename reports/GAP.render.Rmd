...
Since the "glpk" approach (as well as the "idol_bab" approach) do not have relevant parameter values for "with_heuristics", "smoothing_factor", "farkas_pricing", "clean_up_threshold" and "branching_on_master", we set these values to `NA`.

```{r}
idol[idol$solver == "glpk",]$with_heuristics = NA
idol[idol$solver == "glpk" | idol$solver == "idol_bab",]$smoothing_factor = NA
idol[idol$solver == "glpk" | idol$solver == "idol_bab",]$farkas_pricing = NA
idol[idol$solver == "glpk" | idol$solver == "idol_bab",]$clean_up_threshold = NA
idol[idol$solver == "glpk" | idol$solver == "idol_bab",]$branching_on_master = NA
```

Then, we concatenate the parameter values with the method's name to obtain a unique identifier for each tested method. The unique identifier can be `glpk`, `idol_bab_h<A>` or `idol_bap_h<A>_s<B>_f<C>_c<D>_m<E>` where
...
idol[idol$solver == "glpk",]$solver_with_params = "glpk"
...
for (solver_with_params in unique(results$solver_with_params)) {
  mismatch = compare_results(results, "glpk", solver_with_params)
  if (nrow(mismatch) > 0) {
...
```{r}
performance_profile(results[results$solver_with_params %in% c("glpk", "idol_bab_h1", "idol_bap_h1_s0_f0_c1500_m1", "coluna"),], xlim = c(1, 200), main = "External solver, Coluna, BaB and BaP")
```
...
```{r}
performance_profile(results[results$solver != "glpk" & results$solver != "idol_bab",], xlim = c(1, 10), main = "idol versus coluna (up to 10)")
performance_profile(results[results$solver != "glpk" & results$solver != "idol_bab",], xlim = c(1, 200), main = "idol versus coluna (up to 200)")
```
